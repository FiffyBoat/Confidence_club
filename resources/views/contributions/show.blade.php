@extends('layouts.app')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-3">
    <h2 class="mb-0">Contribution Details</h2>
    <div class="d-flex gap-2">
        @if($contribution->receipt)
        <a href="{{ route('receipts.view', $contribution->receipt) }}" class="btn btn-primary" target="_blank">View / Print</a>
        <a href="{{ route('receipts.download', $contribution->receipt) }}" class="btn btn-outline-primary">Download Receipt</a>
        @endif
        <a href="{{ route('contributions.index') }}" class="btn btn-outline-secondary">Back</a>
    </div>
</div>

<div class="card shadow-sm border-0">
    <div class="card-body">
        <div class="row g-3">
            <div class="col-md-4">
                <div class="text-muted small">Member</div>
                <div class="fw-semibold">{{ $contribution->member->full_name ?? '-' }}</div>
            </div>
            <div class="col-md-4">
                <div class="text-muted small">Type</div>
                <div class="fw-semibold">{{ $contribution->type }}</div>
            </div>
            <div class="col-md-4">
                <div class="text-muted small">Description</div>
                <div class="fw-semibold">{{ $contribution->description ?? '-' }}</div>
            </div>
            <div class="col-md-4">
                <div class="text-muted small">Amount</div>
                <div class="fw-semibold">GHS {{ number_format($contribution->amount, 2) }}</div>
            </div>
            <div class="col-md-4">
                <div class="text-muted small">Payment Method</div>
                <div class="fw-semibold">{{ strtoupper($contribution->payment_method) }}</div>
            </div>
            <div class="col-md-4">
                <div class="text-muted small">Transaction Date</div>
                <div class="fw-semibold">{{ $contribution->transaction_date?->format('Y-m-d') }}</div>
            </div>
            <div class="col-md-4">
                <div class="text-muted small">Receipt</div>
                <div class="fw-semibold">{{ $contribution->receipt->receipt_number ?? 'Pending' }}</div>
            </div>
        </div>
    </div>
</div>
@endsection
